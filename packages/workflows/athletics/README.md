...
A component of [msar](https://www.npmjs.com/package/msar): Download athletics stats from for a CSV file of teams

![NPM Version](https://img.shields.io/npm/v/@msar/athletics)
...
The URL of the LMS instance

#### <u>`pathToCsv`</u>

Path to a CSV file containing a column labeled `Lead Group Id`
...
Download schedule data (Default: `true`, use `--no-schedule` to disable)

#### `--results`

Download competition results data (Default: `true`, use `--no-results` to disable)

#### `--stats`

Download team stats data (Default: `true`, use `--no-stats` to disable)
